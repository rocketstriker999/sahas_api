        ...
        req.body.data.forEach((element) => {
            mediaInsertionPromises.push(
                executeSQLQueryParameterized(
                    "INSERT INTO MEDIA(downloadable,type,view_index,title, media_group_id, source) SELECT ?,?,?,?, media_group_id, ? FROM CHAPTERS WHERE id = ?",
...
router.post("/demo-media", async (req, res) => {
    if (req.body.data) {
        ...
        req.body.data.forEach((element) => {
            mediaInsertionPromises.push(
                executeSQLQueryParameterized(
                    "INSERT INTO MEDIA(downloadable,type,view_index,title, media_group_id, source) SELECT ?,?,?,?, media_group_id, ? FROM SUBJECTS WHERE id = ?",
