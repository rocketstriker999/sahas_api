...
function getSubjectsByCourseId(courseId) {
    return executeSQLQueryParameterized(
        `SELECT  table_subjects.id , table_subjects.title from MAPPING_COURSE_SUBJECTS table_mapping_course_subjects INNER JOIN SUBJECTS table_subjects ON table_mapping_course_subjects.subject_id=table_subjects.id where table_mapping_course_subjects.subject_id=?`,
        [courseId]
    ).catch((error) => {
