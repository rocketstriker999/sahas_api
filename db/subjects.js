...
function getAllSubjects() {
    return executeSQLQueryParameterized(
        `SELECT 
            COURSE_SUBJECTS.*,
            (SELECT COUNT(*) FROM SUBJECT_CHAPTERS WHERE SUBJECT_CHAPTERS.subject_id = COURSE_SUBJECTS.id AND active=TRUE) AS chapters_count,
            ...
            (SELECT COUNT(*) FROM MEDIA WHERE media_group_id = COURSE_SUBJECTS.media_group_id AND type='PDF' AND active=TRUE) AS demo_pdfs_count
        FROM 
            COURSE_SUBJECTS
        WHERE 
            active = TRUE
