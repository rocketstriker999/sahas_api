...
router.post(
    "/",
    async (req, res, next) => {
        const requiredBodyFields = ["category_id", "title", "image", "fees", "view_index"];
    ...
    async (req, res, next) => {
        if (!!(await getCoursesContainerByCategoryIdAndTitle(req.body))) {
            return res.status(400).json({ error: "Courses Container Already Exist" });
        }
        next();
    },
    async (req, res) => {
        const courseId = await addCoursesContainer(req.body);
        res.status(201).json(await getCoursesContainerById({ id: courseId }));
    }
);
