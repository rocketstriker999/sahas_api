...
function getCouponCodeCoursesByCouponCodeId({ coupon_code_id }) {
    return executeSQLQueryParameterized(
        `SELECT COUPON_CODE_COURSES.*,COURSES.title,USERS.full_name as distributor_name FROM COUPON_CODE_COURSES LEFT JOIN COURSES ON COUPON_CODE_COURSES.course_id=COURSES.id  LEFT JOIN ON USERS WHERE COUPON_CODE_COURSES.email=USERS.email WHERE COUPON_CODE_COURSES.coupon_code_id=?`,
        [coupon_code_id]
    ).catch((error) => {
...
function getCouponCodeCoursesByIds({ couponCodeCourseIds }) {
    return executeSQLQueryParameterized(
        `SELECT COUPON_CODE_COURSES.*,COURSES.title,USERS.full_name as distributor_name FROM COUPON_CODE_COURSES LEFT JOIN COURSES ON COUPON_CODE_COURSES.course_id=COURSES.id LEFT JOIN ON USERS WHERE COUPON_CODE_COURSES.email=USERS.email WHERE COUPON_CODE_COURSES.id in (${couponCodeCourseIds})`
    ).catch((error) => {
